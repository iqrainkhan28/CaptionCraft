...
from text_speech_utils 
import record_audio, transcribe_audio, speak
from streamlit_extras.switch_page_button 
import switch_page
...
# ==========================================================
# >>> ADDED BACKGROUND IMAGE (ONLY THIS PART IS NEW) <<<
# ==========================================================
st.markdown("""
<style>
body {
...
if st.button("Start Listening"):
    try:
        ...
        record_audio(audio_path)  # Save voice input
        user_text = transcribe_audio(audio_path)  # Convert speech to text
        st.success(f"You said: {user_text}")

        translated_text = GoogleTranslator(source='en', target=selected_lang_code).translate(user_text)
        st.markdown(f"**Translated Caption:** {translated_text}")

        speak(translated_text)  # Optional: speak it out loud
